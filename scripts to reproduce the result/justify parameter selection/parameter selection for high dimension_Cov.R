...
#for (j in 1:5){
#j<-1
S4_Sim_high_cov<-function(j2,k){
  wcs<-mclapply(1:num_simulation,function(ii){
    k_vector<-2:7
    #q = c(5,10) #number of DE features out of 1000
    ...
    #h<-1000#DE evidence
    set.seed(ii)
    x<-Sim3(POIN1=POIN1,POIN2=POIN2,POIN3=POIN3,Nm=POIM,
            M=10,Ua=Ua,Ub=Ub,Ulow=ulow[k],Uupper=uupper[k],sigma1=sigma1,sigma2=sigma2,Nnoise=Nnoise,cov=cov_list[j2])
    true_feature<-x$true_feature
    ...
    # 
     res<-list(result_K_sens_0=result_K_sens_0,result_K_spec_0=result_K_spec_0,result_K_youden_0=result_K_youden_0,
              ...
              result_feature_youden=result_feature_youden,true_label=true_label,true_feature=true_feature,x=x,wbounds_list=wbounds_list)
    #res<-list(wbounds_list=wbounds_list,x=x)
    return(res)
...
result_high_cov<-list()
result_high_cov[[1]]<-S4_Sim_high_cov(1,1)
result_high_cov[[2]]<-S4_Sim_high_cov(1,2)
result_high_cov[[3]]<-S4_Sim_high_cov(2,1)
result_high_cov[[4]]<-S4_Sim_high_cov(2,2)
result_high_cov[[5]]<-S4_Sim_high_cov(3,1)
result_high_cov[[6]]<-S4_Sim_high_cov(3,2)

save(result_high_cov,file="Sim_S4_high_cov.Rdata")
...
load("Sim_S4_high_cov.Rdata")
#load("Sim_S4_high_cov_wbound_list.Rdata")

res.k<-matrix(NA,16,6)
res.jaccard<-matrix(NA,16,6)
...
for(i in 1:6){
  ...
  for(j in 1:50){
    ...
    temp3<-list()
    temp4<-list()
    wbound_list<-result_high_cov[[i]][[j]]$wbounds_list
    #x<-x$x
    true_feature<-result_high_cov[[i]][[j]]$true_feature
    true_label<-result_high_cov[[i]][[j]]$true_label
    for(k in 1:6){
      result_k[[k]]<-unlist(result_high_cov[[i]][[j]]$result_K_youden_0.05[[k]])
      result_lambda[[k]]<-unlist(result_high_cov[[i]][[j]]$result_feature_youden[[k]])
    ...
    ari1<-adjustedRand(true_label,res1[[1]]$Cs)[[2]]
    jaccard1<-Jaccard.index(true_feature,as.numeric(res1[[1]]$ws>0))
    num1<-sum(res1[[1]]$ws>0)
    
    res2<-KMeansSparseCluster(x,K=est_k,wbounds=wbound2,nstart=50)
    ari2<-adjustedRand(true_label,res2[[1]]$Cs)[[2]]
    jaccard2<-Jaccard.index(true_feature,as.numeric(res2[[1]]$ws>0))
    num2<-sum(res2[[1]]$ws>0)
    
    res3<-KMeansSparseCluster(x,K=est_k,wbounds=wbound3,nstart=50)
    ari3<-adjustedRand(true_label,res3[[1]]$Cs)[[2]]
    jaccard3<-Jaccard.index(true_feature,as.numeric(res3[[1]]$ws>0))
    num3<-sum(res3[[1]]$ws>0)
    
    res4<-KMeansSparseCluster(x,K=est_k,wbounds=wbound4,nstart=50)
    ari4<-adjustedRand(true_label,res4[[1]]$Cs)[[2]]
    jaccard4<-Jaccard.index(true_feature,as.numeric(res4[[1]]$ws>0))
...
res.jaccard<-round(res.jaccard,3)
res.k<-round(res.k,3)
res.numfeature<-round(res.numfeature,3)

res<-rbind(res.ari[1:4,],res.jaccard[1:4,],res.numfeature[1:4,],res.k[1:4,])

write.csv(res,file="Combine_two_scores_cov.csv")
...
for(i in 1:6){
  ...
  for(j in 1:50){
    ...
    for(k in 1:6){
      ...
      #----------------------no trim and sens
      result_k1[[k]]<-unlist(result_high_cov[[i]][[j]]$result_K_sens_0[[k]])
      result_lambda1[[k]]<-unlist(result_high_cov[[i]][[j]]$result_feature_sens[[k]])
      temp1[[k]]<-0.5*result_k1[[k]]+0.5*result_lambda1[[k]]
      #----------------------trim 5% and sensitivity
      result_k2[[k]]<-unlist(result_high_cov[[i]][[j]]$result_K_sens_0.05[[k]])
      result_lambda2[[k]]<-unlist(result_high_cov[[i]][[j]]$result_feature_sens[[k]])
      temp2[[k]]<-0.5*result_k2[[k]]+0.5*result_lambda2[[k]]
      #----------------------no trim youden
      result_k3[[k]]<-unlist(result_high_cov[[i]][[j]]$result_K_youden_0[[k]])
      result_lambda3[[k]]<-unlist(result_high_cov[[i]][[j]]$result_feature_youden[[k]])
      temp3[[k]]<-0.5*result_k3[[k]]+0.5*result_lambda3[[k]]
      #----------------------trim 5% and youden
      result_k4[[k]]<-unlist(result_high_cov[[i]][[j]]$result_K_youden_0.05[[k]])
      result_lambda4[[k]]<-unlist(result_high_cov[[i]][[j]]$result_feature_youden[[k]])
...
res<-round(res,3)
write.csv(res,file="Combine_two_scores_cov_sens_and_trim.csv")
...
for(i in c(1:6)){
  ...
  boxplot(DF, col = rainbow(2, s = 0.5),cex.axis=3,ylim=c(0.9,1.02))
  #plot(density(res.youden),col="black",main="density plot",ylim=c(0,5),xlim=c(0,1))
  #title1<-paste("=",q[i],", u=",u[i],sep="")
  # plot(density(res.sens),col="black",main="density plot",
