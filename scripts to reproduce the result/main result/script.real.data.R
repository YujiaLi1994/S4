...
set.seed(12315)
res.ps<-KL.PS(x,lambda_list=wbounds_list,cv=2,k_vector=k_vector,M=n.div,num.cores=10,cutoff = 0.8)
...
set.seed(12315)
res.ps<-KL.PS(x,lambda_list=wbounds_list,cv=2,k_vector=k_vector,M=n.div,num.cores=20,cutoff = 0.8)
result.ps = KMeansSparseCluster(x, K=res.ps$res_k, wbounds = res.ps$res_w, nstart = 100, ######
                                 silent = FALSE, maxiter=6, centers=NULL)
