...
Ub<-10
j1<-1
j2<-3#the parameter needs to be tuned
Nnoise<-600
...
for (k in 1:2){
  for(ii in 1:num_simulation){
    q = c(10) #number of genes modules
    ...
    for(l in 1:length(k_vector)){
      wbounds_list[[l]] = region.lambda(lam1=3,iteration=40,x=x,K=k_vector[l])
    }
    ...
    ari_gap<-adjustedRandIndex(true.label,clus_label_gap)
    ...
    ari_s4<-adjustedRandIndex(true.label,clus_label_s4)
    ...
    res.ps = KL.PS(x,lambda_list=wbounds_list,cv=2,k_vector=k_vector,M=n.div,num.cores=num_cores,cutoff = 0.8)
    ...
    ari_ps<-adjustedRandIndex(true.label,clus_label_ps)
